...
// ─────────────────────────────────────────────────────────────────────────────
// LockTracker  –  "set of enemies pending a LeapFrog hit" approach
//
// Lifecycle
//   Init  (move start, first RefillTargetsAvailable):
//     Add ALL enemies in the target list, regardless of locks.
//
//   Hit landed (HitData.SetQTEResult, filtered to LeapFrog/HeapFrog moves):
...
//     If the enemy still has LeapFrog-matching locks → re-add to the set.
...
static class LockTracker
{
    ...
    /// Called once at move start (first RefillTargetsAvailable).
    /// </summary>
    ...
    internal static void OnHitApplied(HitData hitData)
    {
        try
        {
            ...
            Plugin.LogI(
                $"[LockTracker] LeapFrog hit 0x{ptr:X} → removed (pending={EnemiesPendingHit.Count})");
        }
        catch (Exception ex)
        {
    ...
    /// <summary>
    /// Re-adds the enemy if they still have matching LeapFrog locks — meaning
    ...
    internal static void UpdateForEnemy(EnemyCombatActor enemy)
    {
        ...
        {
            IntPtr ptr = enemy.Pointer;
            bool hasLocks = EnemyHasMatchingLocks(enemy, LeapFrogCycleFlag.MoveDamageTypes);

            if (hasLocks) EnemiesPendingHit.Add(ptr);

            Plugin.LogI(
                $"[LockTracker] OnLocksChanged 0x{ptr:X}: " +
                $"leapFrogLocks={hasLocks} | pending={EnemiesPendingHit.Count}");
        }
        catch (Exception ex)
        {
...
/// Re-adds the enemy to EnemiesPendingHit if they still have matching LeapFrog locks.
/// </summary>
...
/// Removes the hit enemy from EnemiesPendingHit (LeapFrog/HeapFrog moves only).
