...
// ─────────────────────────────────────────────────────────────────────────────
// LockTracker  –  "set of enemies pending a Moonrang/Soonrang hit" approach
//
// Lifecycle
//   Init  (move start, first RefillAvailableTargetsLeft):
//     Add ALL enemies in the target list, regardless of locks.
//
//   Hit landed (HitData.SetQTEResult, filtered to Moonrang/Soonrang moves):
...
//     If the enemy still has Moonrang-matching locks → re-add to the set.
//     If no matching locks remain → leave them removed.
//
//   Grant deflect:
//     While the set is non-empty.
//     Force stop (return without deflecting) when empty.
// ─────────────────────────────────────────────────────────────────────────────
...
static class LockTracker
{
    ...
    internal static void OnHitApplied(HitData hitData)
    {
        try
        {
            ...
            Plugin.LogI(
                $"[LockTracker] Moonrang/Soonrang hit 0x{ptr:X} → removed (pending={EnemiesPendingHit.Count})");
        }
        catch (Exception ex)
        {
    ...
    /// <summary>
    /// Re-adds the enemy if they still have matching Moonrang locks — meaning
    ...
    internal static void UpdateForEnemy(EnemyCombatActor enemy)
    {
        ...
        {
            IntPtr ptr = enemy.Pointer;
            bool hasLocks = EnemyHasMatchingLocks(enemy, MoonrangCycleFlag.MoveDamageTypes);

            if (hasLocks) EnemiesPendingHit.Add(ptr);

            Plugin.LogI(
                $"[LockTracker] OnLocksChanged 0x{ptr:X}: " +
                $"moonrangLocks={hasLocks} | pending={EnemiesPendingHit.Count}");
        }
        catch (Exception ex)
        {
...
/// Re-adds the enemy to EnemiesPendingHit if they still have matching Moonrang locks.
/// </summary>
...
/// Removes the hit enemy from EnemiesPendingHit (Moonrang/Soonrang moves only).
...
static class Patch_HitData_SetQTEResult_LockTracking
{
    static void Prefix(HitData __instance)
    {
        ...
        if (name.Contains("Moonrang") || name.Contains("Soonrang"))
